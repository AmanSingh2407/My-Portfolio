...
export default function Home() {
  return (
    <section className="relative mx-auto flex min-h-[calc(100vh-5rem)] w-full max-w-6xl flex-col justify-center gap-12 px-6 py-20">
      <div className="pointer-events-none absolute inset-0 -z-10 bg-gradient-to-br from-accent/10 via-transparent to-slate-900/50" />
      <motion.div initial="hidden" animate="visible" transition={{ duration: 0.8 }} variants={heroVariants} className="space-y-8">
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          <Link to="/contact" className="inline-flex items-center gap-2 rounded-full border border-slate-800/80 px-6 py-3 text-sm text-slate-200 transition hover:border-accent hover:text-accent">
            Let’s Collaborate
          </Link>
        </div>
      </motion.div>
      <motion.ul initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6, duration: 0.8 }} className="grid gap-6 text-sm text-slate-400 sm:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          <li key={item.label} className="rounded-2xl border border-slate-800/60 bg-slate-900/30 p-6">
